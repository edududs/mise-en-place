...
from mise_en_place.restaurant import Clock, Section


def test_renaming_cook_preserves_role() -> None:
    house = build_restaurant(Clock(minute_s=1), rng=random.Random(7))
...
def test_zero_clock_returns_finite_measurement() -> None:
    result = build_restaurant(Clock(minute_s=0), rng=random.Random(7)).measurement("teste")
